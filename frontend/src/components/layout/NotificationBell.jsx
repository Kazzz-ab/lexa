...
const TYPE_COLOR = { urgent: '#DC2626', court: '#1B3A6B', invoice: '#B45309' };
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ background: 'var(--surface)', boxShadow: '0 20px 60px rgba(27,58,107,0.18)', border: '1px solid rgba(201,168,76,0.2)' }}>
            <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
              ...
              <button onClick={() => setOpen(false)} className="text-[#6B6B7B] hover:text-[#1B3A6B]">
                <X size={14} />
              </button>
            </div>
            <div className="max-h-72 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="py-8 text-center">
                  <Bell size={24} className="mx-auto mb-2 text-[#C9A84C]/30" />
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--muted)', fontStyle: 'italic' }}>All clear — no alerts</p>
                </div>
              ) : notifications.map((n) => {
                const Icon = TYPE_ICON[n.type] || Bell;
                const color = TYPE_COLOR[n.type] || '#1B3A6B';
                return (
                  <button key={n.id} onClick={() => { setOpen(false); navigate(n.href); }}
                    className="w-full flex items-start gap-3 px-4 py-3 hover:bg-[#F5F2ED] transition-colors text-left"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.75rem', color: 'var(--muted)', fontStyle: 'italic' }}>
                        {n.body}
                      </p>
                    </div>
                  </button>
