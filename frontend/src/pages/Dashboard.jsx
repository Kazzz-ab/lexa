...
const statusConfig = {
  active: { label: 'Active', color: '#4C1D95', bg: 'rgba(76,29,149,0.1)' },
  open: { label: 'Open', color: '#7C3AED', bg: 'rgba(46,95,163,0.1)' },
  pending: { label: 'Pending', color: '#B45309', bg: 'rgba(180,83,9,0.1)' },
  closed: { label: 'Closed', color: '#6B6B7B', bg: 'rgba(107,107,123,0.1)' },
  'on-hold': { label: 'On Hold', color: '#5B21B6', bg: 'rgba(139,92,246,0.1)' },
};
const priorityDot = { low: '#6B6B7B', medium: '#7C3AED', high: '#B45309', urgent: '#DC2626' };
...
export default function Dashboard() {
  ...
  const statCards = stats ? [
    { label: 'Active Clients', value: stats.activeClients.toLocaleString(), change: 'on record', icon: Users, color: '#4C1D95', bg: 'rgba(76,29,149,0.08)', trend: 'up' },
    { label: 'Attorneys', value: stats.totalAttorneys, change: 'all active', icon: Scale, color: '#8B5CF6', bg: 'rgba(139,92,246,0.08)', trend: 'neutral' },
    { label: 'Open Cases', value: stats.openCases, change: `${stats.caseBreakdown?.active || 0} active`, icon: Briefcase, color: '#7C3AED', bg: 'rgba(46,95,163,0.08)', trend: 'up' },
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="mb-10">
        <div className="flex items-end gap-3 flex-wrap">
          ...
          <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: 'var(--muted)', marginBottom: '0.2rem' }}>Esq.</span>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {[1,2,3,4].map(i => (
            <div key={i} className="rounded-xl p-5 animate-pulse" style={{ background: 'var(--surface)', border: '1px solid rgba(139,92,246,0.15)', height: 120 }} />
          ))}
        </div>
      ) : (
            ...
            <motion.div key={stat.label} variants={item}
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="w-11 h-11 rounded-lg flex items-center justify-center float-anim border"
                  style={{ background: stat.bg, borderColor: 'rgba(139,92,246,0.15)' }}>
                  <stat.icon size={19} style={{ color: stat.color }} />
                </div>
                <span className="text-xs font-medium px-2.5 py-1 rounded-md"
                  style={{
                    background: stat.trend === 'up' ? 'rgba(76,29,149,0.08)' : stat.trend === 'down' ? 'rgba(180,83,9,0.08)' : 'rgba(139,92,246,0.1)',
                    color: stat.trend === 'up' ? '#4C1D95' : stat.trend === 'down' ? '#B45309' : '#5B21B6',
                    fontFamily: 'var(--font-body)',
                    border: '1px solid',
                    borderColor: stat.trend === 'up' ? 'rgba(76,29,149,0.12)' : stat.trend === 'down' ? 'rgba(180,83,9,0.12)' : 'rgba(139,92,246,0.2)',
                  }}>
                  {stat.change}
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.55 }}
          ...
          style={{ background: 'var(--surface)', border: '1px solid rgba(139,92,246,0.15)', boxShadow: '0 2px 12px rgba(76,29,149,0.07)' }}>
          <div className="px-6 py-5 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
            <div>
              <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '1rem', color: 'var(--text)' }}>Active Matters</h2>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.77rem', color: 'var(--muted)' }}>
          ...
          <div className="divide-y" style={{ borderColor: 'rgba(139,92,246,0.08)' }}>
            {loading ? (
              <div className="py-10 text-center text-[#6B6B7B]" style={{ fontFamily: 'var(--font-body)' }}>Loading…</div>
            ) : recentCases.length === 0 ? (
              <div className="py-10 text-center text-[#6B6B7B]" style={{ fontFamily: 'var(--font-body)' }}>No active matters</div>
            ) : recentCases.map((c, i) => {
              const sc = statusConfig[c.status] || statusConfig.open;
              return (
                <motion.div key={c._id}
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 + i * 0.07 }}
                  className="px-6 py-4 flex items-start gap-4 hover:bg-[#F4F3F9] transition-colors cursor-pointer">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.55 }} className="space-y-4">
          <div className="rounded-xl p-5"
            style={{ background: 'linear-gradient(135deg, #4C1D95 0%, #7C3AED 100%)', border: '1px solid rgba(139,92,246,0.3)', boxShadow: '0 8px 32px rgba(76,29,149,0.25)' }}>
            ...
            <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.77rem', color: 'rgba(139,92,246,0.8)', marginTop: '0.3rem' }}>
          ...
          <div className="rounded-xl p-5"
            ...
            {[
              { label: 'Active', key: 'active', icon: CheckCircle2, color: '#4C1D95' },
              { label: 'Pending Review', key: 'pending', icon: Clock, color: '#8B5CF6' },
              ...
              <div key={s.label} className="flex items-center gap-3 mb-3.5 last:mb-0">
                ...
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: 'var(--muted)', flex: 1 }}>{s.label}</span>
