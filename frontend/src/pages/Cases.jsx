...
const STATUS_COLORS = {
  open: { bg: 'rgba(46,95,163,0.1)', color: '#7C3AED', label: 'Open' },
  active: { bg: 'rgba(76,29,149,0.1)', color: '#4C1D95', label: 'Active' },
  pending: { bg: 'rgba(180,83,9,0.1)', color: '#B45309', label: 'Pending' },
  closed: { bg: 'rgba(107,107,123,0.1)', color: '#6B6B7B', label: 'Closed' },
  'on-hold': { bg: 'rgba(139,92,246,0.1)', color: '#5B21B6', label: 'On Hold' },
};

const PRIORITY_COLOR = { low: '#6B6B7B', medium: '#7C3AED', high: '#B45309', urgent: '#DC2626' };
...
function RowMenu({ caseData, onEdit, onClose: onCloseCase }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className="p-1.5 rounded-lg text-[#6B6B7B] hover:bg-[#F4F3F9]">
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.83rem', color: 'var(--muted)', marginTop: '0.2rem' }}>{total} matters on record</p>
        </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid rgba(139,92,246,0.15)', boxShadow: '0 2px 12px rgba(76,29,149,0.07)' }}>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Briefcase size={32} className="mx-auto mb-3 text-[#8B5CF6]/30" />
            <p style={{ fontFamily: 'var(--font-body)', color: 'var(--muted)' }}>No matters found</p>
          </div>
        ) : (
            ...
            {cases.map((c) => {
              ...
              return (
                <motion.div key={c._id}
                  ...
                  className="grid grid-cols-12 px-6 py-4 items-center hover:bg-[#F4F3F9] transition-colors"
                  style={{ borderBottom: '1px solid rgba(139,92,246,0.06)' }}>
                  <div className="col-span-1" style={{ fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: '#8B5CF6', fontWeight: 600 }}>{c.caseNumber}</div>
                  <div className="col-span-4">
                    <p style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '0.87rem', color: 'var(--text)' }}>{c.title}</p>
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.73rem', color: 'var(--muted)' }}>{c.practiceArea}</p>
                  </div>
                  <div className="col-span-2" style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--text)' }}>
                    {c.client?.firstName} {c.client?.lastName}
                  </div>
                  <div className="col-span-2" style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--muted)' }}>
